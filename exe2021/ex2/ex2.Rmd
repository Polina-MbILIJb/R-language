...
- класс $$Y=0: X \sim N((25, 17),\begin{pmatrix} 8^2 &0\\0&11^2\end{pmatrix})$$

- класс $$Y=1: X \sim N((23, 39),\begin{pmatrix} 10^2 &0\\0&12^2\end{pmatrix})$$
...
![](plot2_1.png)
По графику можно заметить, что найдутся такие точки, которые принадлежат не своему классу (5 желтых и 6 синих).
...
Так и есть. Точность на обучающей выборке близка к идеальной (`r Acc`).
...
**Вывод**: Высокая точность не является гарантией качества модели. На обеих тестовых выборках точность оказалась одинаковой и не очень высокой, поэтому выбор метода является затруднительным. Выберем метод ближайшего соседа для дальнейших расчётов характеристик качества.
...
Вывод: Таким образом, характеристики качества TPR, SPC, PPV, NPV и MMC желательно должны быть приближены к единице, в то время как ошибки - быть минимальными. По тестовой выборке мы можем увидеть, что она плохо описывает обучающую выборку и совершенно ей не соответствует.
