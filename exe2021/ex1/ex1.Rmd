...
```
<!--Я ПЫТАЛАСЬ СДЕЛАТЬ ДЛЯ 2 ЧАСТИ ФУНКЦИЮ, ЧТОБЫ ИЗМЕНЯТЬ ЛИШЬ ПАРАМЕТРЫ DF, SD И НАЗВАНИЕ PNG ФАЙЛА, В КОТОРЫЙ БУДЕТ СОХРАНЯТЬСЯ ФАЙЛ, НО ТАК И НЕ СМОГЛА РАЗОБРАТЬСЯ С ВОЗВРАЩЕНИЕМ ГРАФИКОВ И СОХРАНЕНИЕМ РИСУНКА В ФУНКЦИИ :( ПОЭТОМУ, К СОЖАЛЕНИЮ, ПРОСТО КОПИРОВАНИЕ ОДНОГО И ТОГО ЖЕ НЕСКОЛЬКО РАЗ (ЭТО ЗЛИТ). 
Частично закомментировала сохранение, чтобы не засорять картинками память вашего компьютера и графики можно просто посмотреть в отчёте, ну или же раскомментировать. Ещё проблема, что при сохранении картинки в отчёте появляются комментарии ##png##2 - какие способы скрытия этого?
...
$$X \sim U(5, 105)$$
$$Y = f(X) + \epsilon, где f(X) = 19 - 0.05 \cdot x; \epsilon \sim N(0,1).$$
...
#dev.off()
## ЗДЕСЬ ВЫВОДИТСЯ В ОТЧЁТЕ РЕШЁТКА КАК УБРАТЬ - ПОДУМАТЬ
```
<!--![](plot1_1.png)-->
...
- При движении слева направо MSE на тестовой выборке (красная кривая) сначала медленно сокращается, затем растёт. Нам известна истинная форма связи Y с X, она описывается линейной функцией. Число степеней свободы такой модели равно числу оцениваемых параметров, т.е. 2 (коэффициент перед X и константа). MSE на тестовой выборке довольно долго остаётся стабильной, а затем начинает расти. Этот рост объясняется эффектом переобучения модели: она всё лучше описывает обучающую выборку, и при этом постепенно становится неприменимой ни к одному другому набору наблюдений.

Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 8 и равно 0,62645. Визуально по графику мы можем установить, что первое значение MSEТЕСТ, близкое к стабильно низким, соответствует df = 2. Ошибка здесь меньше 1, что ненамного отличается от минимума. Именно df = 2 было выбрано в качестве компромисса между точностью (минимальной MSE на тестовой выборке) и простотой модели (чем меньше степеней свободы, тем модель проще). При выборе df = 7 ошибка составляет ~ 4% от среднего Y, а при 2 она не превышает 5%, так что прямая - оптимальный вариант.
...
Неустранимая ошибка модели равняется 2, можно обратить внимание, что рассеивание точек стало больше. Ошибка обучающей выборки ниже неустранимой ошибки только при степени свободы больше 20. 

Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 8 и равно 2,506. Визуально по графику мы можем установить, что первое значение MSEТЕСТ, близкое к стабильно низким, соответствует df = 7. Ошибка здесь больше 2, что отличается от минимума. Именно df = 7 было выбрано в качестве компромисса между точностью (минимальной MSE на тестовой выборке) и простотой модели (чем меньше степеней свободы, тем модель проще). При выборе df = 7 ошибка составляет ~ 15% от среднего Y.



```{r}
res.sd <- 3              # стандартное отклонение случайного шума
df.choose <- 7           # выбрать для графика 2 оптимальную степень свободы
...
Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 8 и равно 5,638. Визуально по графику мы можем установить, что первое значение MSEТЕСТ, близкое к стабильно низким, соответствует df = 7. Ошибка здесь больше 3. При выборе df = 7 ошибка составляет ~ 34% от среднего Y.

```{r}
res.sd <- 4              # стандартное отклонение случайного шума
df.choose <- 7           # выбрать для графика 2 оптимальную степень свободы
...
Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 8 и равно 10,023. Визуально по графику мы можем установить, что первое значение MSEТЕСТ, близкое к стабильно низким, соответствует df = 7. Ошибка здесь больше 4. При выборе df = 7 ошибка составляет ~ 58% от среднего Y.

**Выводы:**

При возрастании неустранимой ошибки процент ошибки выборки относительно среднего быстро растёт, что не есть хорошо. Тестовая выборка отдаляется от неё, а обучающая пересекает при увеличении числа степеней свободы. Подобрать подходящую модель становится более затруднительным. Стоит ли использовать смещение?
